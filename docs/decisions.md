...
**Atualização 2026-07-07 — os 4 bugs foram corrigidos, mais 4 achados adicionais que estavam mascarados.** Sessão de debugging sistemático (commits `909d251` e `f5d6253`) confirmou causa-raiz de cada um dos 4 bugs via RED→GREEN real contra Postgres (Testcontainers) antes de corrigir:

1. **Filtro de tenant (`TenantFilterEvaluatableExpressionFilter`)** impedia a tradução SQL de qualquer query em entidade tenant-scoped ao bloquear a funcletização de `ContaId` — removido; comportamento padrão do EF Core restaurado (a exceção fail-closed `TenantNaoResolvidoException` agora chega como `InnerException` de `InvalidOperationException`, não mais solta). Também corrigido bug relacionado: `BeginSystemScope` não fazia bypass automático do filtro nomeado `"Tenant"` — agora faz, via curto-circuito `IsSystemScope || ContaId == e.ContaId`.
2. **`AddOutboxInbox<TDbContext>`** registrava o dispatcher/retenção só via `AddHostedService<T>()` (expõe apenas `IHostedService`) — passaram a ser singleton concreto também.
3. **`AuditoriaTestFixture`** chamava `EnsureCreatedAsync` duas vezes contra o mesmo banco físico — a segunda chamada era ignorada (EF acha que o banco já existe) e a tabela `auditoria.registro_auditoria` nunca era criada. Trocado por `GenerateCreateScript`+`ExecuteSqlRaw`.
4. **`IdempotencyDecisionRules.Decidir`** nunca checava `ExpiraEm` — único bug real de **produção** encontrado numa segunda rodada de investigação (os outros 3 achados dessa rodada eram bugs no harness de teste: dupla-registro de handler no DI, hash plantado incorreto, `HttpClient` descartado antes da requisição paralela terminar).

**Resultado:** suíte de integração foi de 36/60 falhando para 59/60 passando. O único item remanescente é um flake de concorrência pré-existente e já conhecido em `Observability/OtelTests` (intermitente, não determinístico, não bloqueado por nenhuma das causas acima).

**Decisão de continuidade do dono (2026-07-07):** com a suíte estável, marcar o job `build-and-test` do `ci.yml` como check obrigatório no branch protection do `master` — pendente apenas de o repositório sair do plano gratuito do GitHub (branch protection para repositório privado exige GitHub Pro/Team ou tornar o repo público; `gh api repos/.../branches/master/protection` retornou 403 confirmando o bloqueio). Nome exato do check a marcar: `build-and-test` (é job único, não dá pra isolar só o step `test-integracao`).
